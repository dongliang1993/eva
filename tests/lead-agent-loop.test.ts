...
const isFinish = (event: AgentStreamEvent): event is FinishEvent => event.type === "finish";
        ...
        return {
          stream: simulateReadableStream({
            chunks: [
              ...
              { type: "tool-call", toolCallId: "tc-1", toolName: "echo", input: JSON.stringify({ msg: "line1\nline2" }) },
...
describe("工具循环", () => {
  it("【回归】工具输出原样透出,没有 JSON 二次转义", async () => {
    const agent = createAgent({ model: toolThenTextModel(), tools: [echoTool()] });
      ...
      doStream: async () => ({
        stream: simulateReadableStream({
          chunks: [
            ...
            { type: "tool-call", toolCallId: "tc-1", toolName: "fail", input: JSON.stringify({}) },
            { type: "finish", finishReason: "tool-calls", usage }
          ]
        })
      })
  ...
  it("两个 step:调工具 → 拿结果 → 输出文本(SDK 驱动 loop,无需手写 for-step)", async () => {
    ...
    const stepStarts = events.filter((e) => e.type === "step-start") as Array<{ step: number }>;
      ...
      doStream: async () => {
        ...
        return {
          stream: simulateReadableStream({
            chunks: [
              { type: "stream-start", warnings: [] },
              { type: "tool-input-start", id: `tc-${calls.count}`, toolName: "echo" },
              { type: "tool-call", toolCallId: `tc-${calls.count}`, toolName: "echo", input: JSON.stringify({ msg: "x" }) },
              { type: "finish", finishReason: "tool-calls", usage }
            ]
          })
        };
        ...
        if (calls.count === 1) {
          return {
            stream: simulateReadableStream({
              chunks: [
                { type: "stream-start", warnings: [] },
                { type: "tool-call", toolCallId: "tc-1", toolName: "echo", input: JSON.stringify({ msg: "x" }) },
                { type: "finish", finishReason: "tool-calls", usage }
              ]
            })
          };
...
describe("max-output 续写", () => {
  ...
  it("shouldContinueForMaxOutput: length + 未超限 → 续写;超限或非 length → 不续写", async () => {
    const { shouldContinueForMaxOutput, MAX_OUTPUT_CONTINUATION_MESSAGE } = await import("../packages/harness/src/agents/context-strategy.js");
    const policy = {
