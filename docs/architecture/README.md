# `docs/architecture/` 阅读导航

> **先看这三行,再决定读哪篇 —— 这个目录里的编号是写作顺序,不是阅读顺序,也不表达「哪篇还有效」。**
>
> - **`00`–`05` / `16`–`21` 是 Alma 竞品逆向研究**,不是 Eva 的架构。`00`–`05` 是 v0.0.175 快照,
>   `16`–`21` 是 v0.0.990 的修订(以后者为准)。想了解 Eva 怎么工作,这十二篇都不是入口。
> - **`07`–`15`、`22`–`25` 是 Eva 自身的设计与落地计划**,其中相当一部分是**当时的目标**,
>   不是今天的代码。逐篇对照代码之前不要把它们当事实。
> - **今天的事实源是仓库根的 [`AGENTS.md`](../../AGENTS.md)**;正在进行的架构改造方案见
>   [`25-eva-simple-architecture-charter.md`](./25-eva-simple-architecture-charter.md)。
>   宪章 §7.21 计划把当前事实收进 `docs/current/` 三份文档(architecture / run-lifecycle /
>   data-ownership)—— 那个目录**还不存在**,建成前请以 AGENTS.md 为准。
