...
| 任务 | 文档 | 内容 | 估时 | 依赖 |
|---|---|---|---|---|
| **T32** | `T32-system-integration.md` | 系统集成：托盘 + Alt+Space 全局唤起 + `eva://` 深链 + 窗口状态记忆 + 自启动开关 + 清 `isQuitting` 死码 | 1 天 | ✅ **运行时验收过**（2026-08-23，含托盘崩溃修复） |
...
>
> **验收状态**：`pnpm typecheck` 全绿（desktop/web/server/harness/shared）；`pnpm run build:electron`（electron-vite）过；`pnpm test` 553 全绿（唯一 error 是基线 `run-detach.test.ts` flake）。~~**`pnpm desktop:build` 在 `rebuild:native` 步失败**~~ → **已解**（`@electron/rebuild` 3.7.2 升 4.2.0，Node 26 ESM 坑消失）。**运行时验收已过（2026-08-23，dev 态）**：`desktop:dev:all` 起 dev 逐项过——托盘（含关窗后点击重建）、Alt+Space 唤起/隐藏、窗口记忆、`eva://settings` 深链跳设置均正常。期间修一个崩溃 bug：mac 关窗留进程后 `mainWindow` 仍指死对象，托盘点击 `focusMainWindow` → `Object has been destroyed`；修法 = `focusMainWindow`/`toggleMainWindow` 判 `isDestroyed()` 重建 + `createWindow` 挂 `closed` 置空 + `activate` 复用 `focusMainWindow`（见 `T32-system-integration.md` 坑）。自启动开关需重启 Mac 验，未单独过（开关读写走 OS API，逻辑简单）。
| **T33** | `T33-security-hardening.md` | 安全收口：CSP 响应头（static.ts）+ loopback token（main 生成→env 传 server→preload 注入 renderer→fetch/SSE 带 header） | 1–1.5 天 | ✅ **打包态实测通过**（2026-08-23） |
...
>
> **验收状态**：`pnpm typecheck` 全绿；`pnpm test` 553 全绿（同前 flake）——测试直接 `buildApp` 无 env，hook 跳过校验，故无回归。**打包态实测通过（2026-08-23）**：curl 打包态 fork 的 prod server（端口动态，非 dev 的 8082）——静态页带 CSP 头 ✓、`/v1/health` 白名单无 token 200 ✓、无 token `POST /api/v1/runs/stream` 返 **401** ✓、错误 token 也 401 ✓。注意：**别拿 dev server（8082，无 `EVA_LOOPBACK_TOKEN`）验**——dev 态 hook 跳过会返 zod 400 而非 401，易误判成 token 没生效。桌面内对话正常一项随 T34 E2E 一并跑过（更新后 app 正常对话）。
...
| **T35** | `T35-dev-bootstrap.md` | dev 一键拉起：`concurrently`+`wait-on` 串起 server+web+desktop | 0.5 天 | ✅ **运行时验收过**（2026-08-23） |
...
>
> **验收状态（2026-08-23 实测通过）**：`pnpm desktop:dev:all` 一条命令拉起三进程——server(8082)/web(5173)/electron 窗口全起、vite 200、server `/v1/health` 200、窗口加载不白屏；wait-on 在 server/web 就绪前挡住 desktop 生效。**注意两个前置坑（本次实测踩到）**：① 若已有手动起的 dev server 占 8082 / vite 占 5173，desktop:dev:all 里 web 会退到 5174 而 desktop 默认连 5173 → 白屏，跑前先 `pkill -f "tsx watch|vite|Electron"` 清场；② **若曾对主 workspace 直接跑过不带 `--module-dir` 的 `electron-rebuild`，`better_sqlite3.node` 会被编成 Electron ABI，系统 node 一加载就 `Killed: 9`(SIGKILL)**，dev server 在「SOUL.md loaded」后静默崩——解法：进 `node_modules/.pnpm/better-sqlite3@*/node_modules/better-sqlite3` 跑 `npm run build-release` 重编回 node ABI（打包用的 `.server-deploy` 那份由 `rebuild:native --module-dir .server-deploy` 单独编，互不干扰）。端口写死 8082/5173，自定义端口走原 `desktop:dev`。
