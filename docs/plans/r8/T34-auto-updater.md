...
10. **prerelease 要 `allowPrerelease=true`**（2026-08 实测）：内测期 release 都是 GitHub **prerelease**，而 electron-updater 默认只认 `/releases/latest`（latest 不含 prerelease）→ `checkForUpdates` 406。内测期 `autoUpdater.allowPrerelease = true`；转正式发正式 release 后改回 false。
11. **mac 自更新必须发 zip，dmg 不行**（2026-08 实测）：dmg 是安装器，`quitAndInstall` 自动替换只认 zip。`electron-builder.yml` target 只配 dmg 时 `checkForUpdates` 报 "ZIP file not provided"。mac target 要 dmg + zip 都发（zip 给 updater，dmg 给用户手动装）。
12. **打包前必须重跑 `pnpm web:build`**（2026-08 实测）：electron-builder 的 extraResources 直接拷 `apps/web/dist`。只重编 dist-electron 而没重 build web 时，dist 停在旧 bundle——若旧 bundle 还没有新 IPC（如 `getUpdaterStatus`），设置页「重启更新」按钮不显示，但 main/IPC/状态全对（CDP `getUpdaterStatus()` 返回正常），极易误判成 main 进程 bug。改完 web 端 updater 相关代码，发版/打包前一律 `pnpm web:build` 再 builder。
