...
/**
 * 危险工具审批决策接口。
 * - GET  /api/tool-approvals         列出全部待审批
 * - POST /api/tool-approvals/:callId  提交决策 granted/denied
 * 前端在 SSE 的 tool_approval 事件后, 用户点按钮 → POST 这里。
 */
export const registerApprovalRoutes = (app: FastifyInstance): void => {
  app.get("/api/tool-approvals", async () => {
